...
const createConfig = async (packageDir, volumeSupport = true) => {
  ...
  let wpContent = `${packageDir}/wp-content`;
  ...
  let pluginThemeVolumes = [
    ...(plugins.map((x) => x.volume)),
    ...(themes.map((x) => x.volume)),
  ];

  if (!config.wpContentVolume) {
    volumes = [...volumes, ...pluginThemeVolumes];
  } else {
    wpContent = pathToWPContent;
  }

  if (config['wp-content']) {
    volumes = [`${wpContent}:/var/www/html/wp-content`, ...volumes];
  }
  ...
  if (config['wp-content']) {
    shell.sed('-i', 'WP_CONTENT', `true`, `${packageDir}/Dockerfile`);
  } else {
    shell.sed('-i', 'WP_CONTENT', `false`, `${packageDir}/Dockerfile`);
  }
