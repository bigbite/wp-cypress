...
const WP_CONTENT_EXCLUDE_PATHS = ['uploads', 'upgrade'];
...
const createConfig = async (packageDir, hasVolumeSupport = true) => {
  ...
  const schema = await fs.readJSON(`${packageDir}/lib/schemas/config.json`);
  ...
  if (config.wpContent) {
    ...
    const ignore = WP_CONTENT_EXCLUDE_PATHS.map(path => `${pathToWPContent}/${path}`);
    ...
    if (hasVolumeSupport) {
      ...
      volumes.push(`${pathToWPContent}:/var/www/html/wp-content`);
    }
