...
const createConfig = async (packageDir, hasVolumeSupport = true) => {
  ...
  if (!config.url) {
    config.url = config.port ? `http://localhost:${config.port}` : 'https://localhost:4433';
  }
  ...
  volumes.push(
    `${CWD}/${config.sslCertificatesPath[0]}:/var/www/html/localhost.pem`,
    `${CWD}/${config.sslCertificatesPath[1]}:/var/www/html/localhost.key`,
  );
