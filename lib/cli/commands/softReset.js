...
const softReset = async (packageDir, logFile, options = false) => {
  ...
  await run(
    async () => wpcli('db export /tmp/db-pre-reset.sql --add-drop-table', logFile),
    'Exporting pre-reset database',
    'Pre-reset Database exported',
    logFile,
  );
  ...
  await run(
    async () => wpcli('db export /tmp/db-post-reset.sql --add-drop-table', logFile),
    'Exporting post-reset database',
    'Post-reset Database exported',
    logFile,
  );
  ...
  await run(
    async () => wpcli('db export /tmp/db-post-export.sql --add-drop-table', logFile),
    'Exporting post-export database',
    'Post-export Database exported',
    logFile,
  );
