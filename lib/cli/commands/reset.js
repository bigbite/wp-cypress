...
const reset = async (packageDir, logFile, options) => {
  ...
  await run(
    async () => wpcli(
      'core config --dbhost=db --dbname=wordpress --dbuser=root --dbpass="" --locale=en_US --extra-php',
      logFile,
...
`,
  ...
  await run(
    async () => wpcli(
      `core ${getWPInstallType(config.multisite)} --url=${config.url} --title="WP Cypress" --admin_user=admin --admin_password=password --admin_email="admin@test.com" --skip-email`,
      logFile,
    ),
