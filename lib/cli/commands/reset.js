...
`, logFile),
  ...
  if (config.multisite && config.multisite == 'subdomains') {
    installType = 'multisite-install --subdomains';
  ...
  await run(
    async () => wpcli(
      `core ${installType} --url=${config.url} --title="WP Cypress" --admin_user=admin --admin_password=password --admin_email="admin@test.com" --skip-email`,
      logFile,
    ),
  ...
  await run(
    async () => wpcli(`plugin activate wp-cypress ${plugins} ${config.multisite ? '--network' : ''}`, logFile),
    'Activating plugins',
    'Activated plugins',
    logFile,
    false,
