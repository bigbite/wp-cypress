...
const config = fs.readJsonSync(`${process.cwd()}/cypress.json`);

// Set ENV variable to tell if we are in no docker, can also
// be set in the config. CIs are likely to use ENV variable directly.
if (!process.env.WP_CYPRESS_NO_DOCKER_MODE) {
  if (config.wp.noDocker) {
    process.env.WP_CYPRESS_NO_DOCKER_MODE = config.wp.noDocker;
  } else {
    process.env.WP_CYPRESS_NO_DOCKER_MODE = false;
  }
}
