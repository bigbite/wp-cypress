...
module.exports = async (_on, config) => {
   ...
   * container.
   */
  if (packageConfig['wp-content'] && !packageConfig['wpContentVolume']) {
  ...
  let tempIntegrationFolder = `${packageDir}/tmp`;
  ...
  if ( ! packageConfig.disableIntegrationClone ) {
    watchDir(config.integrationFolder, duplicateTests);
  } else {
    tempIntegrationFolder = config.integrationFolder;
  }

  let baseUrl =  packageConfig.port ? `http://localhost:${packageConfig.port}` : 'http://localhost';
